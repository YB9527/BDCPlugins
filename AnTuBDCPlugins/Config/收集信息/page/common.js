...
function openBdcWin(o) {
    ...
    if (!!d && !!d.url) {
        ...
        if (d.url.indexOf("/") > -1 || d.url.indexOf(".html") > -1) {
            ...
            // Common.SetCookie("WHERE", Common.Encode(escape(filter)));
            ...
            OpenTopDialogMinButton(Common.GetBasePath(url), d.buttonName, eval(d.callBack), d.popCallBack, null, null, st, ct, null, null, customselectmode);
...
//上饶那边需要最小化按钮，OpenTopDialogShangRao和Common.OpenTopDialog区别就是多了最小化按钮
function OpenTopDialogMinButton(url, title, cb, f, o, c, st, ct, w, h, cselectmode) {
...
function openGA() {
    var qlrList = $("div[tabname='JJ_QLR[QLRDIRECT=0]']").siblings('div[dtype="dataRow"]');
    for (var i = 0; i < qlrList.length; i++) {
        var zjhm = $("div[tabname='JJ_QLR[QLRDIRECT=0]']").siblings('div[dtype="dataRow"]').eq(i).find('input[col="ZJHM"]').val();
        var qlrmc = $("div[tabname='JJ_QLR[QLRDIRECT=0]']").siblings('div[dtype="dataRow"]').eq(i).find('input[col="QLRMC"]').val();

        var param = {
            "bus_sre_businesstypename": "首次登记",
            "name": qlrmc,
            "idCardNum": zjhm,
            "bus_sre_appname": "正式系统",
            "xzqh": "511400",
            "bus_sre_businesstypecode": "SC000023",
            "businessNumber": "21"
        };
        $.ajax({
            type: "POST",
            contentType: "application/json;charset=utf-8",
            url: "http://172.242.15.81:8888/identityverification/identityverificationprovider/getAuditResults",
            dataType: 'json',
            cache: false,
            async: true,
            ...
            error: function (message) {
                top.Dialog.confirm(message, function () {
                    ...
                    function () {
                        top.Toast("showNoticeToast", "点击了取消")
                    });
            },
            success: function (res) {
                if (res.status == 1) {
                    var result = res.result.ResourceInfos[0].ReturnInfos[0].CheckCodeDesc;
                    top.Dialog.alert(result, function () {
                    })
                } else {
                    top.Dialog.alert(res.message, function () {
                    })
                }
            }
        });
...
function openMZ() {
    ...
    for (var i = 0; i < qlrList.length; i++) {
        ...
        diag.URL = Common.GetBdcPath() + "/Page/OneFormHtmls/mzInformation.html?zjhm=" + zjhm + "&qlrmc=" + qlrmc;
        diag.show();
    }
}
...
$.extend(Common, {
    ...
    downTemplateFile: function (url, fileName) {
        ...
        xhr.open('post', "/ANTU.BDCDJ.XTGL.PROVIDER/importFlie/downLoadModel");
        // if("fwfghbdz"==fileName || "xzmbdz"==fileName){
        //     xhr.open('post', "../../../importFlie/downLoadModel");
        // }
        ...
        xhr.onload = function (e) {
            if ((xhr.status >= 200 && xhr.status < 300) || xhr.status === 304) {
                ...
                var filename = Global.GetConfig(fileName);
                if (!filename) {
                    filename = "下载文件" + new Date().getTime();
                }
                ...
                switch (fileName) {
                    case "jtgxmb" : filename = "家庭关系模板" + ext;
                }
    ...
    getOneFormData: function (detailId, bsm, cfxxid, cdetailId) {
        ...
        $.ajax({
            ...
            data: {
                ...
                args: [Common.GetCookie("token"), detailId, bsm, !!cfxxid ? cfxxid : '', cdetailId, fsiteId, hasrightRegionId]
            },
            async: false,
            dataType: 'json',
            success: function (data) {
                if (data != null) {
                    ...
                    var fdcjyjg = Common.GetCookie("fdcjyjg");
                    //如果备案号获取的fdcjyjg不为空就表示是备案信息，直接替换房地产交易价格
                    if (fdcjyjg != "" && rs.assetList != undefined && rs.assetList.length > 0) {
                        rs.assetList[0].fdcjyjg = fdcjyjg / 10000;
                        result = JSON.stringify(rs);
                        // result = result.replaceAll(rs.assetList[0].fdcjyjg,fdcjyjg);
                        Common.SetCookie("fdcjyjg", "");
                    }
                } else {
    ...
    splitDivData: {},
    InitSplitDivEvent: function (splitDivName) {
        this.splitDivData.mainDivWidth = $("#" + splitDivName).width();
        this.splitDivData.dragging = false;
        this.splitDivData.splitDiv = $("#" + splitDivName).find(".splitDiv");
        var vm = this;
        this.splitDivData.splitDiv.bind('mousedown', function () {
            vm.splitDivData.dragging = true;
            vm.splitDivData.leftOffset = $("#" + splitDivName).offset().left;
            vm.startX = window.event.pageX;
        });
        $(document).bind("mousemove", function (e) {
            if (vm.splitDivData.dragging) {
                var clickX = e.pageX;
                var bhl = clickX - vm.startX;
                vm.startX = clickX;
                var beforeChangeLeft = vm.splitDivData.splitDiv[0].offsetLeft;
                if (clickX > vm.splitDivData.leftOffset) {
                    vm.splitDivData.splitDiv.css('left', clickX - vm.splitDivData.leftOffset + 'px');
                    vm.splitDivData.splitDiv.prev().width(clickX - vm.splitDivData.leftOffset + 'px');
                    var nextW2 = clickX - vm.splitDivData.leftOffset;
                    vm.splitDivData.splitDiv.next().width(vm.splitDivData.mainDivWidth - nextW2 - 10 + 'px');
                } else {
                    vm.splitDivData.splitDiv.css('left', '0px');
                    vm.splitDivData.splitDiv.prev().width('0px');
                    vm.splitDivData.splitDiv.next().width(vm.splitDivData.mainDivWidth - 10 + 'px');
                }
                if (clickX >= (vm.splitDivData.leftOffset + vm.splitDivData.mainDivWidth - 10)) {
                    vm.splitDivData.splitDiv.css('left', vm.splitDivData.mainDivWidth - 10 + 'px');
                    vm.splitDivData.splitDiv.prev().width(vm.splitDivData.mainDivWidth - 10);
                    vm.splitDivData.splitDiv.next().width('0px');
                }
            }
        });
        $(document).bind("mouseup", function (e) {
            vm.splitDivData.dragging = false;
            e.cancelBubble = true;
            vm.startX = null;
        });
    }
