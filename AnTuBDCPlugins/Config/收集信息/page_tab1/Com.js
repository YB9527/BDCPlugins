...
function clearNoNum_point(obj, i) {
  clearNoNum(obj);
  if ('2' == i) {
    var pattern = /\.\d{2}/;
    obj.value = obj.value.replace(/\.\d{2,}/g, pattern.exec(obj.value))
  } else if('3' == i){
    var pattern = /\.\d{3}/;
    obj.value = obj.value.replace(/\.\d{3,}/g, pattern.exec(obj.value))
  } else {
    var pattern = /\.\d{4}/;
    obj.value = obj.value.replace(/\.\d{4,}/g, pattern.exec(obj.value))
  }
}
...
function copyToClipboard(txt) {
  if (window.clipboardData) {
    ...
    input.value = txt.replaceAll("<br>", "");  // 设置内容
